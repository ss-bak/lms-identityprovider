...
public class AuthController {
	...
	public ResponseEntity<Void> registerUser(@Valid @RequestBody User newUser, @RequestHeader("Authorization") String requestTokenHeader) {
		try {
			...
			if ((user.getUserRoleSet().contains(new Role("Admin"))) ||
					(user.getUserRoleSet().contains(new Role("Librarian")) && !newUser.getUserRoleSet().contains(new Role("Admin")))){
	...
	public ResponseEntity<Void> deleteUser(@PathVariable String usernameToDelete, @RequestHeader("Authorization") String requestTokenHeader) {
		try {
			if (requestTokenHeader == null || !requestTokenHeader.startsWith("Bearer ")) {
				return ResponseEntity.badRequest().build();
			}
